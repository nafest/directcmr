...
TEST(unordered_list_element, pays_respect_to_item_margin) {
    fake_backend fbcknd;
    auto &ss = fbcknd.get_style_sheet();
    ss.set_float_param("margin_top", 2.f);
    ss.set_float_param("margin_bottom", 2.f);
    ...
    auto doc = dcmr::document::from_string("- Item1\n- Item2");

    doc.set_backend(&fbcknd);
    auto height = doc.layout(500);
    doc.render(dcmr::vec2(), 500);

    auto lh = doc.get_root_element()->get_font(&fbcknd)->get_line_height();

    EXPECT_EQ(2, fbcknd.m_draw_marker_calls.size());
    EXPECT_EQ(6 + 3,
              fbcknd.m_draw_marker_calls[0].m_marker_rect.top_left().y());
    EXPECT_EQ(lh + 6 + 3,
              fbcknd.m_draw_marker_calls[0].m_marker_rect.bottom_right().y());
}
